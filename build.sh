...
make -j$(nproc) all
echo "#define BACKEND_BT 1" > reach/reach_backend_config.h
make BIN_DIR=$(pwd)/../build/bin_bt_cad -j$(nproc) all
echo "#define BACKEND_M5 1" > reach/reach_backend_config.h
make BIN_DIR=$(pwd)/../build/bin_msat -j$(nproc) all
echo "#define BACKEND_Y2 1" > reach/reach_backend_config.h
cd ..
popd
...
python avr.py -n test_btor2_bt -b build/bin_bt_cad examples/btor2/counter.btor2
python avr.py -n test_btor2_msat -b build/bin_msat examples/btor2/counter.btor2
